...
function mockDb(rowsMap) {
  return {
    async query(sql, params) {
      ...
      if (/FROM withdrawals/.test(sql)) return { rows: rowsMap.payouts || [] };
...
describe('monitoring.service', () => {
  it('computes overview aggregates', async () => {
    const db = mockDb({
      ...
      payouts: [
        { status: 'SUCCESS', count: 3, avg_time_sec: 5 },
        { status: 'FAILED', count: 1, avg_time_sec: 15 },
        { status: 'PROCESSING', count: 2, avg_time_sec: 8 },
        { status: 'PENDING', count: 1, avg_time_sec: 3 },
      ],
    ...
    expect(res.payouts.items_processing).toBe(3);
  ...
  it('lists payout failures by default', async () => {
    const db = mockDb({
      payouts: [{ id: 'p1', status: 'FAILED', failure_reason: 'x', amount: 10, created_at: '2020' }],
    });
