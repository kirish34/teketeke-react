...
router.get('/summary', requireUser, async (req, res) => {
  try{
    ...
    const [cashRes, expRes] = await Promise.all([
      ...
      req.supa.from('taxi_expense_entries').select('amount,created_at,category').order('created_at',{ascending:false}),
    ]);
...
router.get('/insights', requireUser, async (req, res) => {
  try{
    ...
    const [cashRes, expRes] = await Promise.all([
      req.supa.from('taxi_cash_entries')
        .select('amount,created_at')
        .gte('created_at', startDate.toISOString())
        .lt('created_at', endDateExclusive.toISOString())
        .order('created_at',{ascending:true}),
      req.supa.from('taxi_expense_entries')
