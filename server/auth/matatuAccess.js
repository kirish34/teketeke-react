...
async function checkMatatuAccess(pool = poolDefault, { user_id: userId, matatu_id: matatuId }) {
  if (!matatuId) return { allowed: false, reason: 'matatu_id_missing', details: {} };
  ...
  if (!matatu) return { allowed: false, reason: 'matatu_not_found', details: {} };

  const userCtx = await resolveUserContext(userId, pool);
  const membershipCtx = await resolveSaccoAuthContext({ userId });
